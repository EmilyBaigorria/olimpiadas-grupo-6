...
<body>
  ...
  if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($result->num_rows == 1) {
      ...
      echo '<label for="obra_social">¿Tiene obra social?</label>';
      echo '<select name="obra_social" id="obra_social">';
      echo '<option value="Si" ' . ($row['obra_social'] == 'Si' ? 'selected' : '') . '>Sí</option>';
      echo '<option value="No" ' . ($row['obra_social'] == 'No' ? 'selected' : '') . '>No</option>';
      echo '</select><br>';
